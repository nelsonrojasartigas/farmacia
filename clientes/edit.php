...
if (isset($_GET['id'])) {
	...
	if (isset($_POST['enviar']) && $_POST['enviar'] == 'si') {
		...
		if (!$nombre) {
            ...
            $mensaje = 'Seleccione el rut';
        }elseif (!$direccion) {
            $mensaje = 'Seleccione la direccion';
        }elseif (!$fechanacimiento) {
            $mensaje = 'Seleccione la fecha de nacimiento';
        }elseif (!$persona) {
			$mensaje = 'Seleccione la persona';
		}else{
			//print_r($id);exit;
			//actualizamos el rol
			$sql = $clientes->editClientes($id, $nombre, $rut, $direccion, $fechanacimiento, $persona);
			//print_r($res);exit;
			if ($sql) {
				$msg = 'ok';
		...
		<div class="row">
			<div class="col-md-6 mt-3">
				...
				<form action="" method="post">
                    ...
                    <div class="form-group">
						<label>Rut</label>
						<input type="text" name="rut" value="<?php echo $res['rut']; ?>" placeholder="Rut" class="form-control">
					</div>

                    <div class="form-group">
						<label>Direccion</label>
						<input type="text" name="direccion" value="<?php echo $res['direccion']; ?>" placeholder="Direccion" class="form-control">
					</div>

                    <div class="form-group">
						<label>Fecha nacimiento</label>
						<input type="date" name="fechanacimiento" value="<?php echo $res['fechanacimiento']; ?>" placeholder="Fecha nacimiento" class="form-control">
					</div>

                    <div class="form-group">
						<label>Persona</label>
						<input type="text" name="persona" value="<?php echo $res['persona']; ?>" placeholder="Persona" class="form-control">
					</div>
